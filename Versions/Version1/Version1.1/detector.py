...
class Detector:
        ...
        if img is not None:
            ...
            for ind in df.index:
                ...
                if label in self.labels:
                    ...
                    self.calculateDensityForArea(img,[x1,y1,x2,y2],label,center_x,center_y,self.main_roi,current_values) 
    ...
    def drawVehicleRectangle(self,img,coords,label:str) :
        ...
        cv2.putText(img, label.upper(), (recx1,recy1), font, font_scale, (0, 255, 0), thickness)

      
        
    def drawRoiCircles(self,image,roi):
        
        corner_points = np.array([[roi.x1,roi.y1],[roi.x2,roi.y2],[roi.x3,roi.y3],[roi.x4,roi.y4],[roi.x5,roi.y5],[roi.x6,roi.y6]])
    ...
    def isInArea(self,centerx,centery,roi):
        polygon = Polygon([(roi.x1,roi.y1), (roi.x2,roi.y2), (roi.x3,roi.y3), (roi.x4,roi.y4), (roi.x5,roi.y5), (roi.x6,roi.y6)])
    ...
    def  calculateDensityForArea(self,img,coords,label,center_x,center_y,roi,current_values):
       ...
       if self.isInArea(center_x,center_y,roi):
            ...
            current_values[roi.name]['total_waiting_time'] , self.density = self.calculate_total_waiting_time(current_values=current_values)
        ...
        for key  in key_list:
            for label in current_values[key]:
                if label != "total_waiting_time":
                    total += current_values[key][label] * self.findVehicleType(label)
