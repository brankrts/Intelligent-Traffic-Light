from contour_finder import ContourModel
import matplotlib.pyplot as plt
import cv2
import numpy as np

class LineCountFinder:
    ...
    def __sort_with_y_values(self):
        ...
        self.groups_max_y = sorted(self.groups_max_y, key=lambda x: x[1] , reverse=True)
        self.groups_min_y = sorted(self.groups_min_y, key=lambda x: x[1], reverse=True)
    ...
    def __draw_lines(self):
        ...
        for i in range(len(self.line_max_count)):
            ...
            if len(self.line_min_count) ==len(self.line_max_count):
                ...
                print(f"Min = {self.line_min_count} - Max = {self.line_max_count}")
        ...
        for point in lines:
            cv2.line(self.contour_model.contour_in_max_contour_area_image, point[0], point[1], (0, 0, 255), 4)
            cv2.line(self.contour_model.line_image, point[0], point[1], (0, 0, 255), 4)
        
        for coord in polygon_coords:
           
            cv2.fillPoly(self.contour_model.contour_in_max_contour_area_image, [coord], color=(0,237,0))
    ...
    def __draw_polygon(self,lines):
        ...
        for i in range(len(sorted_lines) - 1):
            ...
            point = np.array([(x1,y1),(x2,y2),(x3  ,y3 ),(x4 ,y4 )] , dtype=np.int32)
        ...
        x1,y1 = first[0][0] - avg_width if first[0][0] - avg_width > 0 else 5  , first[0][1] 
        x2 , y2 = first[0][0] - offset , first[0][1] 
        x3 , y3 = first[1][0] - offset , first[1][1]  
        x4,y4 = first[1][0] - avg_width if first[1][0] - avg_width > 0 else 5  , first[1][1] 
        
        first_point = np.array([(x1,y1),(x2,y2 ),(x3 ,y3 ),(x4 ,y4 )] , dtype=np.int32)
        
       
        
        x1,y1 = last[0][0]  + offset , last[0][1] 
        x2 , y2 = last[0][0] + avg_width if last[0][0] + avg_width < image_width else image_width -10 , last[0][1] 
        
        x3 , y3 = last[1][0] + avg_width if last[1][0] + avg_width < image_width else image_width -10 , last[1][1] 
        x4,y4 = last[1][0] + offset , last[1][1] 
        
        last_point =  np.array([(x1,y1),(x2,y2 ),(x3,y3 ),(x4 ,y4 )] , dtype=np.int32)
